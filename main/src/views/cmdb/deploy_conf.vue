<template>
  <div class="app-container">
    <el-form ref="listQuery" :model="listQuery" :inline="true">
      ...
      <el-form-item>
        <el-button type="primary" @click="dialogVisibleAdd=true" icon="el-icon-plus" v-if="adminrole">新增记录</el-button>
      </el-form-item>
    </el-form>
...
<script>
import { getRow, addRow, editRow, deleteRow, getAppLocal, getProductAppIntegration, getIpIntegration, addValidation } from '@/api/cmdbs/deploy_conf'
...
export default {
  ...
  created() {
    ...
    getAppLocal().then(response => {
      this.app_local_list = response
    })

    // getAppIntegration().then(response => {
    //   this.app_list = response
    // })

    getIpIntegration().then(response => {
      this.ip_list = response
    })

    getProductAppIntegration().then(response => {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$confirm(msg, '提示', this.confirmOptions).then(() => {
        deleteRow(row.app_rm).then(response => {
          this.$notify({
            title: '成功',
            message: '删除成功',
            type: 'success',
