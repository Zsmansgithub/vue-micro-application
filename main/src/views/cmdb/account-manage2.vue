<template>
  <div class="app-container">
      ...
      <el-form ref="listQuery" :model="listQuery" :inline="true">
        <el-row>
          ...
          <el-form-item prop="user">
            <el-select v-model="listQuery.be_granted_people" clearable filterable style="width:200px" placeholder="被授权人" @change="handleFilter">
              <el-option v-for="item in user_list" :key="item" :label="item" :value="item"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item prop="ip">
            <el-input v-model="listQuery.app_cname" placeholder="应用名" clearable style="width: 140px;" class="filter-item"/>
          </el-form-item>
    ...
    <el-table v-loading="listLoading" :data="list" border stripe highlight-current-row @cell-click="cellClick" :cell-style="cellStyleClass"
      ...
      <el-table-column label="中文应用名" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column align="center" width="65px" label="明文显示">
      ...
      <el-table-column label="授权时间" align="center" prop="expire_time"  sortable>
        <template slot-scope="scope">
          <span>{{ scope.row.update_time }}</span>
        </template>
      </el-table-column>

      <el-table-column label="被授权人/组" width="150px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.user || scope.row.group}}</span>
        </template>
      </el-table-column>

      <el-table-column label="授权人" width="100px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.grant_people }}</span>
        </template>
      </el-table-column>

      <el-table-column label="操作" width="70px" align="center">
        <template slot-scope="scope">
          <el-button type="danger" plain :disabled="scope.row.grant_people != people" icon="el-icon-delete"
                     circle @click="deleteButtonClick(scope.row)"/>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog title="修改账号信息" :visible.sync="dialogFormVisible" style="text-align: left;" width="27%" @open="dialogOpen"
      ...
      <el-form ref="dataForm" :model="accountEdit" :rules="rules" label-width="100px">
        ...
        <el-col :span="24">
          <el-form-item label="IP地址" min-width="100" prop="ip">
            <el-input v-model.trim="accountEdit.ip" type="text" auto-complete="off"
                      style="width:100%"/>
          </el-form-item>
        </el-col>
        <el-col :span="24">
        ...
        <el-col :span="5">
          <div>
            <a class="el-dropdown-link"
               href="/vue/cmdb/%E8%B4%A6%E5%8F%B7%E7%AE%A1%E7%90%86%E6%89%B9%E9%87%8F%E6%A8%A1%E6%9D%BF.xlsx">
    ...
    <el-dialog title="授权给用户" :visible.sync="grantPermissionVisible" style="text-align: left;" width="600px">
      <el-form ref="grantForm" :model="grantForm" label-width="100px">
        <el-table :data="grantForm.grant_table_data" highlight-current-row ref="grantTable"
          ...
          <el-table-column label="员工/工号" align="center" show-overflow-tooltip width="250px">
            <template slot-scope="scope">
              <el-form-item label-width="0" style="margin-bottom:0" :prop="'grant_table_data.' + scope.$index + '.user'" :rules="rules_add.user">
                <el-select v-model="scope.row.user" clearable filterable style="width:100%">
                  <el-option v-for="item in user_list" :key="item" :label="item" :value="item"></el-option>
                </el-select>
              </el-form-item>
            </template>
          ...
          <el-table-column label="过期时间" align="center" show-overflow-tooltip width="250px">
            <template slot-scope="scope">
              <el-form-item label-width="0" style="margin-bottom:0" :prop="'grant_table_data.' + scope.$index + '.expire_time'" :rules="rules_add.expire_time">
...
<script>
  import {
    fetchNumberList2, addNumber, editNumber, deleteNumber, getLoginUser, accountPermissionDelete,
    ...
    methods: {
      ...
      deleteButtonClick(row) {
        ...
        this.$confirm(msg, '提示', this.confirmOptions).then(() => {
          accountPermissionDelete(row.id).then((response) => {
            if (response.error) {
              this.$message({
                title: '失败',
      ...
      clickAddDeviceButton() {
        if (this.grantForm.grant_table_data.length > 0) {
          ...
          this.grantForm.grant_table_data.push({user: null, expire_time: data.expire_time})
        } else {
          this.grantForm.grant_table_data.push({user: null})
        }
      },
      grantPermissionClick() {
        this.grantPermissionVisible = true;
        this.grantForm.grant_table_data = [{user: null}]
      },
      ...
      getList() {
        ...
        fetchNumberList2(this.listQuery).then(response => {
