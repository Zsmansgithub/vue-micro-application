<template>
  <div>
      ...
      <el-form ref="ackForm" :model="ackForm" :rules="rules">
        ...
        <el-form-item v-if="ackForm.hand_name == '沟通'" label="沟通对象" prop="uuid_user" required>
          <el-select v-model="ackForm.uuid_user" filterable clearable placeholder="请输入工号/姓名" style="width: 100%;"
            ref="select" @hook:mounted="cancalReadOnly" @visible-change="cancalReadOnly">
    ...
    <el-col :xs="22" :sm="22" :md="22" :lg="22" :xl="15" :offset="1">
      <div style="padding-top:0.5rem;padding-bottom:3.5rem;" class="scrolldiv">
        ...
        <el-card shadow="never" class="el-card_in_edit" style="border-top:0.2rem #67C23A solid;">
          ...
          <table border="0" cellspacing="0" cellpadding="10" width="100%" style="font-size:0.9375rem;">
            ...
            <tr align="left">
              <td><b>申请人员：</b>{{this.$route.params.detailData.create_user_id}} {{this.$route.params.detailData.create_user_name}}</td>
            </tr>
            <tr align="left">
              <td><b>申请时间：</b>{{this.$route.params.detailData.create_time}}</td>
            </tr>
            <tr align="left">
              <td><b>所在部门：</b>{{this.$route.params.detailData.create_user_department}}</td>
            </tr>
            <tr align="left">
              <td><b>所在岗位：</b>{{this.$route.params.detailData.create_user_post}}</td>
            </tr>
          </table>
        </el-card>
            ...
            <el-timeline :reverse="false">
              <el-timeline-item
                ...
                <el-card :style="{'border-color':item.color}" shadow="never" v-if="item.node_id !== 'end'">
                  <div slot="header" class="clearfix">
                    <div style="display:inline-block;height:3rem;width:3rem;">
                      <img src="../../assets/face.png" style="display:block;height:100%;width:100%;" />
                    </div>
                    <div style="display:inline-block;">
                      <!--处理人-->
                      <div v-if="item.type=='primary'" :title="title">{{item.user_name}} {{item.user}}
                        <el-button type="text" style="padding:0;" icon="el-icon-refresh" @click="refreshuser"></el-button>
                      </div>
                      <div v-else>{{item.user_name}} {{item.user}}</div>
                      <!--处理时间-->
                      <div><i class="el-icon-time"></i>&nbsp;{{item.create_time}}</div>
                      <!--处理时长-->
                      <div><svg-icon icon-class="时长" class="svgstyle"/>
                        <span v-if="item.update_status">未开始</span>
                        <span v-else>{{timeComputed(item.c_time) }}</span>
                      </div>
                    </div>
                  </div>
                  <div>
                    <!--处理意见-->
                    <el-row>
                      <div class="text-wrapper">{{item.comments}}</div>
                    </el-row>
                    <!--附件-->
      ...
      <el-row class="detailtitle" style="width:inherit;"> <!-- 继承父元素宽度，但有侧边栏时对不齐 -->
        ...
        <el-col style="width:calc(100% - 3rem);" align="right">
          ...
          <div v-if="this.$route.params.detailData.node_handle" style="position:relative;">
            ...
            </el-button><el-button type="primary" plain @click="setuldisplay()">更多&nbsp;<i :class="buttonsicon"></i></el-button>
            <ul style="list-style:none;position:absolute;bottom:3.5rem;right:0;" v-show="uldisplay" class="buttonmoreul">
              <li v-for="(item,index) in this.$route.params.detailData.node_handle" v-if="index > 1" @click="open(item)">{{item.name}}</li>
              <li v-if="this.$route.params.detailData.node_handle[0].name == '同意'" @click="handleShowother($route.params.detailData.node_handle[0].ID)">转办</li>
              <li v-if="this.$route.params.detailData.node_handle[0].name == '同意'" @click="updateCommon4($route.params.detailData.node_handle[0].ID)">认领</li>
              <li @click="handleding()">督办提醒</li>
              <li @click="handlepre()"><i class="el-icon-d-arrow-left el-icon--left"></i>上个待办工单</li>
              <li @click="handlenext()">下个待办工单<i class="el-icon-d-arrow-right el-icon--right"></i></li>
              <!-- Redis 到 数据库组实施的节点时，同步到cachecloud -->
              <li v-if="this.$route.params.detailData.status==0 && this.$route.params.detailData.node_id == 125 && this.$route.path=='/workflow/Redis_detail'" @click.native="handlecachecloud()">同步到cachecloud</li>
            </ul>
            <!-- <el-dropdown><el-button type="primary" plain class="buttonmore">
              ...
              <el-dropdown-menu slot="dropdown">
                ...
                <el-dropdown-item @click.native="handlepre()" icon="el-icon-d-arrow-left">上个待办工单</el-dropdown-item>
                <el-dropdown-item @click.native="handlenext()">下个待办工单<i class="el-icon-d-arrow-right el-icon--right"></i></el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown> -->
          </div>
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      if (workflow_detail) {
        ...
        this.$store.dispatch('tagsView/delView', this.$route).then(({ visitedViews }) => {
          ...
          this.$router.push('/workflow_h5/workflow_h5list');
        })
      }
    ...
    methods: {
      ...
      // 跳转申请单管理
      handleReset() {
        // this.$router.push({path:`/workflow_h5/workflow_h5list`})
        this.$store.dispatch('tagsView/delView', this.$route).then(({ visitedViews }) => {
          sessionStorage.removeItem(this.session_name);
          this.$router.push('/workflow_h5/workflow_h5list');
        })
      ...
      setuldisplay(){
        ...
        if (this.buttonsicon == 'el-icon-arrow-up') {
          this.buttonsicon = 'el-icon-arrow-down'
        } else{
          this.buttonsicon = 'el-icon-arrow-up'
        }
      },
      // 点击审批按钮,打开对话框
      open(item){
      ...
      // 解决el-select filterable ios不能弹出键盘
...
<style lang="scss" scoped>
  ...
  .detailtitle{position:fixed;bottom:0;background-color:white;z-index:100;height:3.5rem;line-height:3.5rem;}
  ...
  /deep/.el-timeline-item {padding:0;}
  .buttonmoreul{margin:0;padding:0;border:0.01rem solid #e6ebf5;width:8rem;}
  .buttonmoreul li{height:1.5rem;line-height:1.5rem;color:#1890ff;cursor:pointer;background-color:white;text-align:center;font-size:14px;}
  .scrolldiv{height: calc(100vh - 4rem);overflow: scroll;}
  /* element-ui 下拉框在ios上 位置错乱 */
  /deep/.el-dialog__wrapper {position: absolute;top: 0;right: 0;bottom: 0;left: 0;overflow: auto;margin: 0;width:100%;overflow-y: scroll;}
</style>
<style lang="scss">
