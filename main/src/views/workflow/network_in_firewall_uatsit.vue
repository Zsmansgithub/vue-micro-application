                ...
                <el-table :data="team.dtable">
                  ...
                  <el-table-column label="操作" align="center" min-width="2">
                    <template slot-scope="scope">
                      ...
                      <el-button type="primary" icon="el-icon-plus" v-if="scope.row.status == 1||scope.row.status == 2" circle style="padding:2px;" @click="addDomain2(team,scope.row)">
          ...
          <el-row>
            <el-col :span="12" class="col-1400-8 col-1400-offset-2">
              <el-form-item label="开通时长(天)" :prop="`array[${tindex}]` + '.day'" :rules="{required:true,message:'请输入开通时长',trigger:'blur'}">
                <el-input-number v-model.trim="team.day" controls-position="right" :min="1" :max="30" :precision="0" :controls="false" style="width: 100%;" placeholder="审批完成时为第一天"></el-input-number>
              </el-form-item>
            </el-col>
            <el-col :span="12" class="col-1400-8">
              <el-form-item label="备注描述" prop="desc">
  ...
  export default {
    ...
    methods: {
      ...
      getuatsitip(array){
        ...
        var prodapp = []
        for (const i of array) {
          if (i.resource_env_name == 'UAT环境' || i.resource_env_name == 'SIT环境') {
            uatsitapp.push(i)
          } else{
            prodapp.push(i.ip)
          }
        }
        return {uatsitapp:uatsitapp,prodapp:prodapp}
      },
      handleClosesip() {
        if(this.Siplist !== ''){
          if (this.isIpList.test(this.Siplist)) {
            ...
            getIpList(this.listQuery2).then(response => {
              ...
              ipenvdict = this.getuatsitip(ipdict.app)
              if (ipnoapp.length !== 0 && ipenvdict.prodapp.length !== 0) {
                this.$alert('以下ip没有关联应用,将无法匹配信息,请联系管理员:<br>'+String(ipnoapp).replace( new RegExp( ',' , "g" ), '<br/>' )
                +'<br>以下ip为生产环境:<br>'+String(ipenvdict.prodapp).replace( new RegExp( ',' , "g" ), '<br/>' ),
                {dangerouslyUseHTMLString: true,customClass:'firewall_myalert'}).then(_ => {}).catch(_ => {});
              } else if (ipnoapp.length !== 0) {
                this.$alert('以下ip没有关联应用,将无法匹配信息,请联系管理员:<br>'+String(ipnoapp).replace( new RegExp( ',' , "g" ), '<br/>' ),
                {dangerouslyUseHTMLString: true,customClass:'firewall_myalert'}).then(_ => {}).catch(_ => {});
              } else if (ipenvdict.prodapp.length !== 0) {
                this.$alert('以下ip为生产环境:<br>'+String(ipenvdict.prodapp).replace( new RegExp( ',' , "g" ), '<br/>' ),
                {dangerouslyUseHTMLString: true,customClass:'firewall_myalert'}).then(_ => {}).catch(_ => {});
              }
              // this.getSip(ipdict.app)
              this.getSip(ipenvdict.uatsitapp)
            })
          } else{
            this.$message({
