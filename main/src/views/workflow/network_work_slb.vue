    ...
    <div>
      <el-dialog
        ...
        <span style="font-size: 16px;">
          1.申请人申请映射至办公网的应用必须是安全无漏洞的应用<br/>
          2.申请人应保证映射至办公网的应用已经通过安全部门的安全测试<br/>
          3.当安全部门检测出申请人映射至办公网的应用存在漏洞时，申请人应主动配合进行漏洞修复<br/>
          4.如果申请人不配合安全部门的漏洞检测及修复工作，安全部门有权收回相应映射权限<br/>
          5.我已知悉生产服务器映射至办公网后，服务级别相对较低，不能保证系统的稳定性。
        </span>
        <span slot="footer" class="dialog-footer">
          <el-button @click="handleClose">取 消</el-button>
          <el-button type="primary" @click="dialogagreeVisible = false">同 意</el-button>
        </span>
      </el-dialog>
      <el-form ref="newAppForm" :model="newAppForm" :rules="rules" label-width="130px" class="newAppForm"
        ...
        <el-row>
          <el-col :span="12" class="col-1400-8 col-1400-offset-2">
            <el-form-item label="环境" prop="env" required>
              <el-select v-model="newAppForm.env" popper-class="app-search-class" placeholder="请选择环境"
          ...
          <el-col :span="12" class="col-1400-8">
            <el-form-item label="网络类型" prop="network">
              <el-select v-model="newAppForm.network" placeholder="请选择网络类型" popper-class="app-search-class"
                         style="width: 100%;" disabled>
                <!-- <el-option label="机房内网(内部调用)" value="机房内网(内部调用)"/> -->
                <!-- <el-option label="公网(互联网调用)" value="公网"/> -->
                <el-option label="办公网(办公网访问)" value="办公网(办公网访问)"/>
              </el-select>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="12" class="col-1400-8 col-1400-offset-2">
            <el-form-item label="负载策略" prop="network_rule">
              ...
              <el-select v-model="newAppForm.network_rule" popper-class="app-search-class"
                         style="width: 100%;">
          ...
          <el-col :span="12" class="col-1400-8">
            <el-form-item label="会话保持" prop="session">
              ...
              <el-select v-model="newAppForm.session" popper-class="app-search-class"
                         style="width: 100%;">
        ...
        <el-row>
          <el-col :span="12" class="col-1400-8 col-1400-offset-2">
            <el-form-item label="是否是web应用" prop="web">
              <el-select v-model="newAppForm.web" popper-class="app-search-class"
                         style="width: 100%;">
                <el-option label="否" value="否"/>
                <el-option label="是" value="是"/>
              </el-select>
            </el-form-item>
            <el-form-item  v-if="newAppForm.web == '是'" label="客户端源地址" prop="Source_Address" required>
              <label slot="label">
                <el-tooltip class="item" effect="dark" content="" placement="top-end">
              ...
              <el-select v-model="newAppForm.Source_Address" popper-class="app-search-class"
                         style="width: 100%;" placeholder="如需要,通过header插X-Forwarded-For">
                <el-option label="无需" value="无需"/>
                <el-option label="需要" value="需要"/>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="12" class="col-1400-8">
            <el-form-item label="应用访问路径" prop="url">
              <el-input v-model.trim="newAppForm.url" placeholder="请输入应用访问的完整URL以供验证配置"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="12" class="col-1400-8 col-1400-offset-2">
            <el-form-item  label="健康检查策略" prop="health">
              <label slot="label">
                <el-tooltip class="item" effect="dark" content="" placement="top-end">
                  <div slot="content">四层端口监听：F5探测服务器端口，端口存在则认为服务正常，转发请求，否则停止转发。<br/>
                                      七层健康检查：head get等方式获取协商好的一个文本，有正常返回信息则认为服务正常，转发请求，否则停止转发。</div>
                  <svg-icon class-name="help-svg" icon-class="help" style="width: 18px; height:18px"/>
                </el-tooltip>
                健康检查策略
              </label>
              <el-select v-model="newAppForm.health" popper-class="app-search-class"
                         style="width: 100%;">
                <el-option label="4层端口监听" value="4层端口监听"/>
                <el-option label="ping" value="ping"/>
                <el-option label="7层" value="7层"/>
              </el-select>
            </el-form-item>
            <el-form-item v-if="newAppForm.health == '7层'" label="检查url" prop="check_name" required>
              <el-input v-model.trim="newAppForm.check_name" placeholder="请与网络组协商后填写"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12" class="col-1400-8">
          ...
          <!-- <el-col :span="3" :xs="11"> -->
            <el-form-item size="small" style="text-align: right" class="config_os" label-width="30px">
              <!-- <el-tooltip content="未选择应用或应用已有ip为0" placement="top">
                <el-button type="primary" v-if="newAppForm.app_id == '' || total ==0" @click="step0" disabled>添加应用IP</el-button>
              </el-tooltip> -->
              ...
              <!-- <el-button type="primary" v-if="newAppForm.app_id == '' || total ==0" @click="step0" disabled>添加应用IP</el-button>
              <el-button type="primary" v-else @click="step0" :disabled="disabledbuttonPool">添加应用IP</el-button> -->
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
              ...
              <div>
                <el-table :data="newAppForm.configinfo">
                  ...
                  <el-table-column prop="weight" label="权重"  align="center">
                    ...
                    <template slot-scope="scope">
                      <el-form-item :prop="'configinfo.' + scope.$index + '.weight'" :rules='newAppForm.rules.weight'>
    ...
    <el-dialog :visible.sync="dialogVisible">
      ...
      <div align="center" v-if="active == 1" style="padding-bottom:2%;">
        ...
        <el-table :data="conf" height="320px">
          ...
          <el-table-column prop="port" label="端口"  align="center" min-width="2">
            <template slot="header" slot-scope="scope">
              <label slot="label">端口
                <el-tooltip class="item" effect="dark" placement="top">
                  <div slot="content">若相同IP端口重复,将只保留一条该记录</div>
          ...
          <el-table-column label="操作" align="right" min-width="1">
            <template slot-scope="scope">
              <!-- <el-tooltip content="添加端口" placement="top"> -->
    ...
    methods: {
      ...
      renderDraftsData1() {
        try {
          ...
          this.newAppForm.network = this.aDetail.network
          this.newAppForm.network_rule = this.aDetail.network_rule
          this.newAppForm.session = this.aDetail.session
          this.newAppForm.web = this.aDetail.web
          this.newAppForm.Source_Address = this.aDetail.Source_Address
          this.newAppForm.url = this.aDetail.url
          this.newAppForm.health = this.aDetail.health
          this.newAppForm.check_name = this.aDetail.check_name
