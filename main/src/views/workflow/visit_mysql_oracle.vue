              ...
              <div>
                <el-table :data="newAppForm.configinfo" class="mytable">
                  <el-table-column prop="MorO" align="center" min-width="2">
                    <template slot-scope="scope">
                      <el-form-item :prop="'configinfo.' + scope.$index + '.MorO'">
                        <el-select v-model="scope.row.MorO" style="width: 100%;" class="myselect2" @change="changedbtype(scope.row)">
                  ...
                  <el-table-column prop="permissions" label="权限"  align="center" min-width="3">
                    <template slot-scope="scope">
                      <el-form-item :prop="'configinfo.' + scope.$index + '.permissions'" :rules="{type:'array',required:true,message:'不能为空',trigger:'change'}">
                  ...
                  <!-- <el-table-column label="网络" align="center" min-width="2">
                    <template slot-scope="scope">
                      <el-button type="success" style="padding:2px;" :disabled="scope.row.db.length==0?true:false" @click="$refs.childMethod.setdbcluster(scope.row.MorO,scope.row.db)">
                        <div>开通<br />防火墙</div>
                      </el-button>
                    </template>
                  </el-table-column> -->
                  <el-table-column prop="firewall_Open" label="防火墙"  align="center" min-width="3">
                    <template slot-scope="scope">
                      <el-form-item :prop="'configinfo.' + scope.$index + '.firewall_Open'">
                        ...
                        @change="$refs.childMethod.setdbcluster(scope.row.MorO,scope.row.db,scope.row.firewall_Open)">开通</el-checkbox>
                      </el-form-item>
                    </template>
                  </el-table-column>
                  <el-table-column label="操作" align="center" min-width="2">
...
import {getmysqloracle,getwindowslist} from '@/api/zabbix'
...
export default {
  ...
  created() {
    ...
    getmysqloracle().then(response => {
      ...
      this.configinfodict.dboptions = JSON.parse(JSON.stringify(this.dboptions))
    })
  ...
  methods: {
    ...
    addDomain() {
      ...
      configinfo.status = 2
      configinfo.dboptions = JSON.parse(JSON.stringify(this.dboptions))
    ...
    changedbtype(row){
      ...
      row.permissions = []
      if (row.MorO == 'Oracle') {
        row.dboptions = JSON.parse(JSON.stringify(this.dboptions_oracle))
      } else{
        row.dboptions = JSON.parse(JSON.stringify(this.dboptions))
      }
    },
    windows(){
    ...
    resetForm(formName) {
      ...
      if (sessionData && JSON.parse(sessionData).routeParams && JSON.parse(sessionData).routeParams.detailData_edit) {
        ...
        this.newAppForm.configinfo[0].dboptions = JSON.parse(JSON.stringify(this.dboptions))
