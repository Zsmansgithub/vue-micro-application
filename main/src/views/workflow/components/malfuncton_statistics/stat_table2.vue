<template>
  ...
  <div>
    <el-table v-loading="false" :data="table_data" border stripe highlight-current-row
      ...
      <el-table-column label="影响项目" align="center" show-overflow-tooltip width="200">
...
<script>
  ...
  import {getProjectDetail} from '@/api/workflow/malfunction_statistics'
  ...
  export default {
    ...
    methods: {
      ...
      clickDetailButtom(row) {
        ...
        getProjectDetail({'project': row.project, 'time_range': this.time_range.toString()}).then(response => {
          this.series = response
          this.line_chart_title = row.project
        })
      },
    }
  }
