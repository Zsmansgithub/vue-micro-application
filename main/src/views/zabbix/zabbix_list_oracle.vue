          ...
          <el-input placeholder="比较值" v-model="listQuery.compare_right" class="input_with_select" clearable>
            <div slot="prepend">
              <el-select v-model="listQuery.compare_left" placeholder="比较字段" style="width: 150px;margin-right: 0;" clearable>
                <el-option label="连接数" value="current_processes"></el-option>
                <el-option label="表锁行锁" value="query_lock"></el-option>
                <el-option label="复制延迟" value="dataguard_lag"></el-option>
    ...
    <el-table v-loading="listLoading" :data="list" border stripe highlight-current-row style="width: 100%;"
        ...
        <template slot-scope="props">
          <el-form label-position="left" inline class="demo-table-expand">
            <div v-for="key of Object.keys(props.row)">
              <div v-if="'oracle_json_new' === key">
                <div v-for="json_key of ['custom','query','other']">
                  <w_collapse v-if="json_key==='custom' && props.row[key][json_key]" :title="json_key" :default_expand="true">
                    <div v-for="stuffing in Object.keys(props.row[key][json_key])">
                      <w_short_notes :title="stuffing">
                        <el-form-item label="label" v-for="inner_key in Object.keys(props.row[key][json_key][stuffing])"
                                      style="margin-left: 20px" label-width="350px">
                          <template slot-scope="scope" slot="label">
                            <a @click="clickShowLineChartButton(inner_key, props.row.items_json[inner_key], props.row.ip)">
                          ...
                          <span>{{ props.row[key][json_key][stuffing][inner_key] }}</span>
                        </el-form-item>
                      </w_short_notes>
                    </div>
                  </w_collapse>
                  <w_collapse v-else-if="props.row[key][json_key]" :title="json_key" :default_expand="true">
                    <el-form-item label="label" v-for="inner_key in Object.keys(props.row[key][json_key])" style="width:32%;margin-left: 20px"
                                  :label-width="inner_key.length >32?'375px':'300px'">
                      <template slot-scope="scope" slot="label">
                        <a @click="clickShowLineChartButton(inner_key, props.row.items_json[inner_key], props.row.ip)">
                          <svg-icon icon-class="data1" style="width: 16px; height:16px;color:dodgerblue;"/>
                        </a>&nbsp&nbsp&nbsp&nbsp
      ...
      <el-table-column label="存活" width="80px" align="center">
        <template slot-scope="scope">
          ...
          <el-tag v-else-if="scope.row.alive==0" type="danger">否</el-tag>
          <el-tag v-else="scope.row.alive==='vip'" type="info">未知</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="连接数" width="110px" align="center">
        <template slot-scope="scope">
          <span :style="getCellFontColor(scope.row.current_processes ,'default')">
            {{ scope.row.current_processes==-1?'': scope.row.current_processes}}
          </span>
          <!--          <div style="color: #8492a6; font-size: 10px">-->
          <!--            等待中: {{scope.row.pyora_passive}}-->
          <!--          </div>-->
        </template>
      </el-table-column>
      <el-table-column label="复制延迟" width="110px" align="center">
        <template slot-scope="scope">
          <span :style="getCellFontColor(scope.row.dataguard_lag ,'default')">
            {{ scope.row.dataguard_lag == -1 ? '':scope.row.dataguard_lag }}
          </span>
        </template>
      </el-table-column>
      <el-table-column label="超时表锁行锁" width="110px" align="center">
        <template slot-scope="scope">
          <span :style="getCellFontColor(scope.row.query_lock ,'default')">
            {{ scope.row.query_lock== -1 ? '':scope.row.query_lock}}
          </span>
        </template>
      </el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
      exportAll() {
        window.location.href = process.env.VUE_APP_BASE_API + "/zabbix/zabbix_list_oracle_export_all_oracle";
      },
      ...
      getList() {
        ...
        zabbixListOracleGetList(this.listQuery).then(response => {
