...
<!--          style="width: 300px"></el-cascader>-->
      ...
      <el-form-item>
        <el-select v-model="historySearch.consumer_name_s" placeholder="Consumer名称" filterable clearable
                   class="filter-item" style="width: 250px">
          ...
          <el-table-column type="expand">
            <template slot-scope="props">
              <el-table :data="props.row.topic_detail" border stripe highlight-current-row
                ...
                <el-table-column label="ClientIp" prop="clientip" min-width="40">
                  <template slot-scope="scope">
                    <span v-if="scope.row.clientip!==''" style="color: #1890ff" @click="display_clientip(scope.row.clientip)">点击查看</span>
...
<!--              <span v-if="scope.row.level">{{'大于'+JSON.parse(scope.row.level)['gte_tps']+' | 小于'+JSON.parse(scope.row.level)['lte_tps']}}</span>-->
      ...
      <el-row>
        <el-col :span="24">
          <chart ref="chartd11" id="dchart1" title="" style="width:100%;height:400px;" v-bind:host_id="historySearch"
             url="/midmonitor/topic_history_tables2"/>
        </el-col>
      </el-row>
    </el-dialog>
    ...
    <el-dialog :visible.sync="dialogVisibleConsumer" custom-class="rocketmqd"  width="70%" :title="dtitle" style="text-align: left">
      ...
      <chart ref="dchart33" id="dchart33" title=""  style="width:100%;height:400px;" v-bind:host_id="historySearch"
             url="/midmonitor/consumer_history2"/>
    </el-dialog>
    <el-dialog title="编辑记录" :visible.sync="dialogVisibleComment" width="450px">
      <el-form ref="remarkForm" :model="remarkForm" class="remarkForm" label-width="100px">
        <el-form-item label="用途" style="width: 400px">
    ...
    <el-dialog title="编辑记录" :visible.sync="dialogVisibleConsumerComment" width="450px">
...
<!--        <el-form-item label="报警频率" style="position: relative">-->
<!--          <el-input v-model="commentForm.level" style="width: calc(50% + 3px);">-->
<!--            <template slot="append">小时</template>-->
<!--          </el-input><el-input v-model="commentForm.level" style="width: 50%;position: absolute;left: 50%;top: 0;height: 100%">-->
...
<script>
  import {
    ...
    getConsumerName,
    getTopicName
  } from '@/api/midmonitor/rocketmq_v2'
  ...
  export default {
    ...
    methods: {
      ...
      editRow(row){
        ...
        this.remarkForm.change_user=this.name
      },
      getClusterInfo(){
      ...
      getTopicList(stime) {
        ...
        // this.listQuery.id = this.historySearch.topic_name_s
        // if (this.historySearch.topic_name_s) {
        //   this.listQuery.topic_id = this.historySearch.topic_name_s[this.historySearch.topic_name_s.length-1]
        // }else {
      ...
      getConsumeList() {
        ...
        fetchConsumeTable(this.listQuery).then(response => {
          ...
          // for (const i of this.consumedata) {
          //   console.log(typeof i.level)
          // }
