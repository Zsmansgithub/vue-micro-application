<template>
    <el-select v-model="val" v-bind="$attrs" v-on="$listeners" :popper-class="'projectcategory-sel-option'"
        ...
        <el-option v-for="item of demandVersionOptions" :label="item" :value="item" :key="item">
        </el-option>
    </el-select>
</template>

<script>
    import { demandVersion } from '@/api/qms'
    export default {
        name: 'demandVersion_select',
        props: {
            value: Object | String,
            placeholder: {
                type: String,
        ...
        methods: {
            getList(query) {
                // 项目下拉接口
                this.loading = true;
                demandVersion({ pageSize: 1000, pageNum: 1, project_category: '', project_name: query })
                    .then((res) => {
                        this.loading = false;
                        if (res && res.status * 1 === 0) {
                            this.demandVersionOptions = res.data || [];
                        } else {
